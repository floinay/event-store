    ...
    WHEN 'object' THEN EXISTS (
      ...
      WHERE (
           ...
           AND regexp_replace(lower(key), '[^a-z0-9]', '', 'g') ~
                 '(name|firstname|lastname|email|phone|phonenumber|telephone|telephonenumber|address|token|password|credential|secret|cardnumber|pan|cvv|ssn|socialsecuritynumber|dateofbirth|birthdate|dob)$'
            )
