...
BEGIN
  ...
  INSERT INTO event_store.recovery_cdc_verifications(
    ...
    p_consumer_group,0,clock_timestamp()
...
REVOKE ALL ON FUNCTION event_store.verify_recovery_cdc_cutover(text,text,text,uuid,text,text,bigint) FROM event_store_cdc;
REVOKE ALL ON FUNCTION event_store.verify_recovery_cdc_cutover(text,text,text,uuid,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION event_store.verify_recovery_cdc_cutover(text,text,text,uuid,text,text) TO event_store_cdc;
