...
AS $$
  SELECT CASE jsonb_typeof($1)
    ...
    WHEN 'number' THEN regexp_replace(
      regexp_replace($1 #>> '{}', '(\.\d*?)0+$', '\1'),
      '\.$', ''
    )
    ELSE $1::text
  END
...
ALTER FUNCTION event_store.append_v1(text,text,text,uuid,uuid,text,bigint,jsonb,jsonb)
  RENAME TO append_v1_no_number_payload;
REVOKE ALL ON FUNCTION event_store.append_v1_no_number_payload(text,text,text,uuid,uuid,text,bigint,jsonb,jsonb)
