-- `append_admission_enabled` selects the CDC-protected operating mode.  It is
-- deliberately false for storage-only deployments, so it cannot also mean
-- "stop writes".  Delivery health is the independent, durable write fence
-- used by the service after a Connect/Kafka failure or unsafe promotion.
ALTER TABLE event_store.runtime_config
  ADD COLUMN cdc_delivery_healthy boolean NOT NULL DEFAULT false;

CREATE FUNCTION event_store.set_cdc_delivery_health(p_healthy boolean)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF p_healthy THEN
    ...
    -- The service performs the Connect and Kafka checks immediately before
    -- this call.  Keep the database-side slot/publication proof in the same
    -- operation that reopens admission.
...
REVOKE ALL ON FUNCTION event_store.set_cdc_delivery_health(boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION event_store.set_cdc_delivery_health(boolean) TO event_store_app;
...
BEGIN
  ...
  END IF;
  -- Only the CDC bootstrap principal may call this final bootstrap step, after
  -- it has verified the connector and active replication slot.
  UPDATE event_store.runtime_config
...
BEGIN
  ...
  END IF;
  -- Recovery activation follows a verified, zero-lag Kafka barrier, so it is
  -- the one recovery path allowed to reopen the durable delivery fence.
  UPDATE event_store.runtime_config
     SET append_admission_enabled=true,
         cdc_delivery_healthy=true,
         wal_budget_bytes=p_wal_budget_bytes,
