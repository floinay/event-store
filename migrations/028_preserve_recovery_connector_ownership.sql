ALTER TABLE event_store.runtime_config
  ADD COLUMN cdc_connector_name text NOT NULL DEFAULT 'event-store-live'
  CHECK (cdc_connector_name ~ '^event-store-[a-z0-9-]{1,63}$');
...
BEGIN
  ...
  UPDATE event_store.runtime_config
    ...
    WHERE singleton;
END $$;
REVOKE ALL ON FUNCTION event_store.activate_recovery_cdc_slot(text,text,bigint) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION event_store.activate_recovery_cdc_slot(text,text,bigint) TO event_store_cdc;
